...
public class ChatbotService {
    ...
    // Agrega este método en ChatbotService.java
    ...
    private String procesarConsultaLocal(String pregunta, String username) {
        ...
        if (preguntaLower.matches(".*(mis libros|mis reservas|qu[eé] libros tengo reservados).*")) {
        ...
        if (preguntaLower.matches(".*(mi progreso|cu[aá]ntos libros he le[ií]do|puntos tengo).*")) {
            return dbQueryService.getMiProgreso(username);
        }

        if (preguntaLower.matches(".*(libros disponibles|qu[eé] libros hay|libros f[ií]sicos).*")) {
            return dbQueryService.getLibrosFisicosDisponibles();
        }

        if (preguntaLower.matches(".*(limpiar chat|borrar historial|resetear conversaci[oó]n).*")) {
            limpiarHistorial(username);
            return "🧹 He limpiado mi memoria. ¿En qué puedo ayudarte ahora?";
        }

        java.util.regex.Pattern categoriaPattern = java.util.regex.Pattern.compile("libros de (\\w+)");
        java.util.regex.Matcher categoriaMatcher = categoriaPattern.matcher(preguntaLower);
        if (categoriaMatcher.find()) {
            String categoria = categoriaMatcher.group(1);
            String primeraLetra = categoria.substring(0, 1).toUpperCase();
            String categoriaCapitalizada = primeraLetra + categoria.substring(1);
            return dbQueryService.getLibrosVirtualesPorCategoria(categoriaCapitalizada);
        }
    ...
    public void streamRespuesta(String pregunta, String username, SseEmitter emitter) {
        executor.submit(() -> {
            try {
                ...
                // Primero intentar responder con datos de la base de datos
                ...
                if (respuestaLocal != null) {
                    ...
                    String encoded = Base64.getEncoder()
                            .encodeToString(respuestaLocal.getBytes(StandardCharsets.UTF_8));
                ...
                // Si es "hola" o saludo, saludar sin contexto
                if (pregunta.toLowerCase().matches(".*(hola|buenos días|buenas tardes|qué tal|saludos).*")) {
                    String saludo = "✨ ¡Hola! Soy el asistente de Lectio. Puedes preguntarme sobre libros, reservas, multas o pedir recomendaciones. ¿En qué te ayudo hoy?";
                    agregarAlHistorial(username, "assistant", saludo);
                    String encoded = Base64.getEncoder().encodeToString(saludo.getBytes(StandardCharsets.UTF_8));
                    emitter.send(SseEmitter.event().data(encoded));
                    emitter.send(SseEmitter.event().data("[DONE]"));
                    emitter.complete();
                    return;
                ...
                String systemPrompt =
                        "Eres un asistente de biblioteca llamado Lectio. " +
                                ...
                                "Si te preguntan por algo que mencionaste antes, recuérdalo.";
                ...
                // Llamar a Groq con reintentos
                llamarGroqConReintentos(body, emitter, username, 0);
    ...
    private void llamarGroqConReintentos(Map<String, Object> body, SseEmitter emitter, String username, int intento) {
        final int maxIntentos = 2;

        System.out.println("🔄 Llamando a Groq - Intento " + (intento + 1) + "/" + maxIntentos);
        ...
        webClient.post()
                ...
                .onErrorResume(e -> {
                    ...
                    if (intento + 1 < maxIntentos) {
                        try {
                            Thread.sleep(2000);
                            llamarGroqConReintentos(body, emitter, username, intento + 1);
                ...
                .subscribe(
                            ...
                            try {
                                ...
                                if ("[DONE]".equals(jsonData)) {
                                    ...
                                    if (respuestaCompleta.length() > 0) {
                                        agregarAlHistorial(username, "assistant", respuestaCompleta.toString());
                                    }
                                    emitter.send(SseEmitter.event().data("[DONE]"));
                                    emitter.complete();
                                    return;
                        ...
                        () -> {
                            System.out.println("\n✅ Stream completado en intento " + (intento + 1));
                        }
                );
    }
}